...
import styles from './CardioLineChartComponent.style';


type CardioLineChartComponentProps = {
  data:any,
  startDate:Date,
  endDate:Date,
  filter:number,
...
export const CardioLineChartComponent = (props: CardioLineChartComponentProps) => {
  ...
  useEffect(() => {
    ...
    if(props.filter == 4) //1 month
    {
      ...
      for(let i = 0; i < result.length; i++)
      {
        ...
        totalTime.push(calculateTotalTime(result[i]))
        totalDistance.push(calculateTotalDistance(result[i]))
  ...
  const calculateTotalDistance = (data:any) => {
    let total = 0;
    for(let i = 0; i < data.workouts.length; i++)
    {
      total += data.workouts[i].totalDistance;
    }
  ...
  const calculateTotalTime = (data:any) => {
    ...
    for(let i = 0; i < data.workouts.length; i++)
    {
      total += data.workouts[i].totalTime;
    }
