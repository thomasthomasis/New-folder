...
import styles from './ExerciseBarChartComponent.style';
import {useRealm, useUser} from '@realm/react';
import {ExtraExercises} from '../../../../schemas/ExtraExercisesSchema';
import {BarChart} from 'react-native-gifted-charts';

type CardioExerciseBarChartComponentProps = {
  data: any;
  exerciseId: string;
};

export const ExerciseBarChartComponent = (props: CardioExerciseBarChartComponentProps) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < result.totalDistancesDecimal.length; i++) {
      ...
      let objectTime = {
        value: result.totalTime[i],
        label: '',
        topLabelComponent: () => <Text style={{color: colors.text, fontSize: 18, marginBottom: 6}}>{result.totalTime[i]}</Text>,
        frontColor: coloursArray[i],
        gradientColor: lightenColour(coloursArray[i], 50),
      };
    ...
    setMaxTime(getMaxValue(result.totalTime));
  ...
  //TO-DO Change this!!!!!! it is wrong and does not add up correctly
  const getTotalDistanceAndTime = (data: any) => {
    ...
    for (let i = 0; i < data.length; i++) {
      for (let j = 0; j < data[i].allExercises.length; j++) {
        if (data[i].allExercises[j] != props.exerciseId) {
          continue;
        }
      ...
      for (let j = 0; j < data[i].distance.length; j++) {
        let exercisesArray = data[i].exercise.map(JSON.parse);
        console.log(exercisesArray);

        if (exercisesArray[j].value != props.exerciseId) {
          continue;
        }
        ...
        let distanceArray = JSON.parse(data[i].distance[j]);
        let timeArray = JSON.parse(data[i].time[j]);
        ...
        for (let k = 0; k < exercisesArray.length; k++) {
          let indexOfExercise = exerciseNames.indexOf(exercisesArray[k].value);
          if (totalDistances[indexOfExercise] == undefined) {
            totalDistances[indexOfExercise] = 0;
            totalTime[indexOfExercise] = 0;
          }
          totalDistances[indexOfExercise] = totalDistances[indexOfExercise] + distance;
          totalTime[indexOfExercise] = totalTime[indexOfExercise] + time;
        }
      }
    }
    ...
    return {exerciseNameConversion, totalDistancesDecimal, totalTime};
  };
