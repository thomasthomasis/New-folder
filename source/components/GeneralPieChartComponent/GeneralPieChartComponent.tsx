...
export const GeneralPieChart = (props: GeneralPieChartProps) => {
    ...
    return (
      <View style={[styles.container, shadow.shadow]}>
        {
          props.type == "status" &&
          <Text style={{marginBottom: 10, fontWeight: '500', color: colors.black, fontSize: 15}}>User Status{'\n'}Distribution</Text>
        }
        {
          props.type == "workoutType" &&
          <Text style={{marginBottom: 10, fontWeight: '500', color: colors.black, fontSize: 15}}>Workout Type{'\n'}Distribution</Text>
        }
        ...
        {data.map((item, index) => (
          <View key={new BSON.ObjectID().toString()}>
          {
            ...
            <View style={{ width: 10, height: 10, backgroundColor: item.color, borderRadius: 5, marginRight: 5 }} />
            <Text>{item.name} {((item.workouts/totalWorkouts) * 100).toFixed(2)}%</Text>
            </View>
          }
          </View>
