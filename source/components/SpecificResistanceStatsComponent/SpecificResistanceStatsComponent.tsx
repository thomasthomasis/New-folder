  ...
  return (
    <View style={styles.container}>
      <View style={styles.row}>
        {props.timeline == 'week' && <Text style={styles.title}>Cardio Stats for the Week</Text>}
        {props.timeline.includes('day') && (
          <Text style={styles.title}>
            Cardio Stats for the {props.timeline.split(' ')[1]}
            {calculateDateEnding(props.timeline.split(' ')[1])}
          </Text>
        )}
      ...
      {props.data.map((item: any) => {
        return <StatsLayout key={item._id} dateCreated={item.dateCreated} exercises={item.exercises} reps={item.reps} weights={item.weights} timeline={props.timeline} id={item._id.toString()} />;
      })}
    </View>
  );
};
  ...
  const makeJSONNice = (data: string) => {
    ...
    if (typeof JSON.parse(data) == typeof 'string') {
      ...
      for (let i = 0; i < JSON.parse(data).length; i += 2) {
        ...
        subcomponent.push(
          <Text key={generateRandomId()} style={styles.text}>
            {value} reps
          </Text>,
        );
      ...
      component.push(
        <View key={generateRandomId()} style={styles.containerData}>
          ...
          <View style={styles.tinyBorder}></View>
          <View key={generateRandomId()}>{subcomponentContainer}</View>
        </View>,
      );
  ...
  return (
    <View key={generateRandomId()}>
      ...
      {dataArray.map((item: any) => {
        return <View key={generateRandomId()}>{makeJSONNice(JSON.stringify(item))}</View>;
      })}
    </View>
  );
};
