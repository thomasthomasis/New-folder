...
import styles from './ResistanceLineChartComponent.style';


type ResistanceLineChartComponentProps = {
  data:any,
  startDate:Date,
  endDate:Date,
  filter:number,
...
export const ResistanceLineChartComponent = (props: ResistanceLineChartComponentProps) => {
  ...
  useEffect(() => {
    ...
    if(props.filter == 4) //1 month
    {
      ...
      for(let i = 0; i < result.length; i++)
      {
        ...
        totalReps.push(calculateTotalReps(result[i]))
        totalVolume.push(calculateTotalVolume(result[i])/1000)
      ...
      setDataTotalReps(totalReps)
      setDataTotalVolume(totalVolume)
      setLabels(labels)
    }
  ...
  const calculateTotalVolume = (data:any) => {
    ...
    {
      total += data.workouts[i].totalVolume;
    }

    return total;
  }

  const calculateTotalReps = (data:any) => {
    let total = 0;
    for(let i = 0; i < data.workouts.length; i++)
    {
      total += data.workouts[i].totalReps;
    }
  ...
  const convertFilterToText = (filter:number) => {
    ...
    return (
      ...
      <Text style={{marginBottom: 10, fontWeight: '800', color: colors.black, fontSize: 18,}}>Average Total Volume (Tonnes)</Text>
...
<LineChart
  ...
  <View style={styles.container}>
  <Text style={{marginBottom: 10, fontWeight: '800', color: colors.black, fontSize: 18,}}>Average Total Reps</Text>
  <Text style={{marginBottom: 10, fontWeight: '300', color: colors.black, fontSize: 18,}}>{convertFilterToText(props.filter)}</Text>
    <LineChart
    data={{ 
        labels: labels, 
