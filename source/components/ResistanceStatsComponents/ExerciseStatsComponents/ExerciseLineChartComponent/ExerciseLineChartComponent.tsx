...
type ResistanceExerciseLineChartComponentProps = {
  data:any,
  startDate:Date,
  endDate:Date,
  filter:number,
...
export const ExerciseLineChartComponent = (props: ResistanceExerciseLineChartComponentProps) => {
  ...
  useEffect(() => {
    ...
    if(props.filter == 4) //1 month
    {
      ...
      {
        let totalVolumeAndReps = calculateTotalVolumeAndReps(result[i].workouts)

        workoutAmounts.push(result[i].workouts.length)
        totalReps.push(totalVolumeAndReps.totalReps)
        totalVolume.push(totalVolumeAndReps.totalVolume/1000)
  ...
  const calculateTotalVolumeAndReps = (data:any) => {
