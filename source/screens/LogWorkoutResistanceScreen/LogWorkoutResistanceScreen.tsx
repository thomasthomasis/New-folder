...
export const LogWorkoutResistanceScreen = ({navigation, route}: LogWorkoutResistanceProps) => {
  ...
  const getExerciseName = useCallback(
    ...
    [extraExercises, sections, user.id],
  );
  ...
  const handleAddInput = (formIndex: any) => {
    ...
    const newweightInput = {
      id: forms[formIndex].weightInputs.length + 1,
      value: '',
    };
  ...
  const submitData = () => {
    ...
    createItem(id, exercises, reps, weights, totalReps, decimalTotalVolume, allExercises);
  ...
  const createItem = useCallback(
    (id: BSON.ObjectId, exerciseDataJson: string[], repsDataJson: string[], weightsDataJson: string[], totalReps: number, totalVolume: BSON.Decimal128, allExercises: string[]) => {
  ...
  }, [extraExercises, getExerciseName, sections]);
                ...
                <>
                  {(item.id as string).length > 3 && (
                    <TouchableOpacity
                      ...
                      onLongPress={() => {
                        setSelectedExercise(item.id as string);
      ...
      {selectingExercise && (
        <>
          ...
          <ScrollView style={{width: '100%', backgroundColor: colors.black}}>
            ...
            {searchText.length == 0 && <AccordionView />}
          </ScrollView>
        </>
      )}
      {!selectingExercise && (
                    ...
                    <View style={styles.inputs}>
                      <View style={styles.inputBoxes}>
                        <View style={styles.inputContainer}>
                          ...
                          <TextInput placeholder="" value={form.weightInputs[inputIndex].value} onChangeText={text => handleWeightInputChange(text, formIndex, inputIndex)} style={styles.input} keyboardType="numeric" />
                        </View>
                        ...
                        <View style={styles.inputContainer}>
                          ...
                          {inputIndex > 0 && <Text></Text>}
                          <TextInput placeholder="" value={form.repInputs[inputIndex].value} onChangeText={text => handleRepInputChange(text, formIndex, inputIndex)} style={styles.input} keyboardType="numeric" />
                        </View>
                      </View>
      ...
      <Modal isVisible={visibleOptions} swipeDirection={['down']} onSwipeComplete={onClose} onBackdropPress={onClose} style={styles.modalView}>
        <View style={styles.modalContent}>
          <TouchableOpacity
            onPress={() => {
              onClose();
              goToEditExercise(selectedExercise);
            }}
          ...
          <TouchableOpacity
            onPress={() => {
              handleConfirmDeleteAddedExercise();
            }}
