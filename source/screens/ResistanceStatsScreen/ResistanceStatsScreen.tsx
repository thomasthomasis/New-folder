...
export const ResistanceStatsScreen = ({ navigation, route }: ResistanceStatsScreenProps) => {
    ...
    const goToExerciseStats = (exerciseId:string) => {
      navigation.navigate("ResistanceExerciseStats", { userId:userId, exerciseId:exerciseId })
    }
    ...
    return (
        ...
        }
        {
            !loading &&
            <>
            <View style={{width: '100%', height: 60, display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: colors.black}}>
                    <View style={styles.headerTitle}>
                        <TouchableOpacity onPress={goBack} style={styles.closeButton}>
                            <MaterialCommunityIcons name="arrow-left" size={40} color={'white'}/>
                        </TouchableOpacity> 
                        <Text style={{fontSize: 20, fontWeight: '800', marginLeft: 20, color: 'white'}}>Resistance Stats</Text>
                    </View>
                </View>
                <ScrollView>
                    ...
                    <View style={{minHeight: screenHeight * 2}}>
                        ...
                        <View style={styles.container}>
                            ...
                            <TouchableOpacity style={[styles.card, shadow.shadow]} onPress={() => goToWorkoutView(highestVolumeSession.workoutId)}>
                                <Text style={{fontSize: 20, fontWeight: '500', color: colors.text,}}>Highest Volume Session</Text>
                                <Text style={{fontSize: 40, fontWeight: '900', color: colors.text}}>{highestVolumeSession.largestVolume}KG</Text>
                                <Text style={{fontSize: 15, fontWeight: '400', color: colors.text}}>{highestVolumeSession.date.toDateString()}</Text>
                            </TouchableOpacity>

                            <TouchableOpacity style={[styles.card, shadow.shadow]} onPress={() => goToWorkoutView(highestRepSession.workoutId)}>
                                <Text style={{fontSize: 20, fontWeight: '500', color: colors.text,}}>Highest Rep Session</Text>
                                <Text style={{fontSize: 40, fontWeight: '900', color: colors.text}}>{highestRepSession.mostReps} Reps</Text>
                                <Text style={{fontSize: 15, fontWeight: '400', color: colors.text}}>{highestRepSession.date.toDateString()}</Text>
                            </TouchableOpacity> 
 
                            <TouchableOpacity style={[styles.card, shadow.shadow]} onPress={() => goToWorkoutView(highestVolumeAndRepSet.workoutIdVolume)}>
                                <Text style={{fontSize: 20, fontWeight: '500', color: colors.text,}}>Highest Volume Set</Text>
                                <Text style={{fontSize: 40, fontWeight: '900', color: colors.text}}>{highestVolumeAndRepSet.largestVolume}KG</Text>
                                <Text style={{fontSize: 17, fontWeight: '700', color: colors.text}}>{getExerciseName(highestVolumeAndRepSet.exerciseIdVolume)}</Text>
                                <Text style={{fontSize: 15, fontWeight: '400', color: colors.text}}>{highestVolumeAndRepSet.dateVolume.toDateString()}</Text>
                            </TouchableOpacity>

                            <TouchableOpacity style={[styles.card, shadow.shadow]} onPress={() => goToWorkoutView(highestVolumeAndRepSet.workoutIdReps)}>
                                <Text style={{fontSize: 20, fontWeight: '500', color: colors.text,}}>Highest Rep Set</Text>
                                <Text style={{fontSize: 40, fontWeight: '900', color: colors.text}}>{highestVolumeAndRepSet.largestReps} Reps</Text>
                                <Text style={{fontSize: 17, fontWeight: '700', color: colors.text}}>{getExerciseName(highestVolumeAndRepSet.exerciseIdReps)}</Text>
                                <Text style={{fontSize: 15, fontWeight: '400', color: colors.text}}>{highestVolumeAndRepSet.dateReps.toDateString()}</Text>
                            </TouchableOpacity>

                            <TouchableOpacity style={[styles.card, shadow.shadow]} onPress={() => goToWorkoutView(highestVolumeAndRepSet.workoutIdWeight)}>
                                <Text style={{fontSize: 20, fontWeight: '500', color: colors.text,}}>Heaviest Weight Lifted</Text>
                                <Text style={{fontSize: 40, fontWeight: '900', color: colors.text}}>{highestVolumeAndRepSet.heaviestWeight}KG</Text>
                                <Text style={{fontSize: 17, fontWeight: '700', color: colors.text}}>{getExerciseName(highestVolumeAndRepSet.exerciseIdWeight)}</Text>
                                <Text style={{fontSize: 15, fontWeight: '400', color: colors.text}}>{highestVolumeAndRepSet.dateWeight.toDateString()}</Text>
                            </TouchableOpacity>
