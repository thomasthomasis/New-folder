...
export const EditGroupEventScreen = ({ navigation, route}: EditGroupEventScreenProps) => {
    ...
    useEffect(() => {
        ...
        {
            inputPairs.push({input1: event[0].linkTitles[i], input2: event[0].links[i]})
        }
                ...
                <View style={styles.modalContent}>
                {
                    colorsArray.map((item, index) => (
                        <View key={new BSON.ObjectID().toString()} style={{display: 'flex', flexDirection: 'row', justifyContent: 'center', width: '90%', marginBottom: 10}}>
                            <TouchableOpacity style={{width: '33%', height: 50, borderRadius: 15, backgroundColor: item.light, marginLeft: 2.5, marginRight: 2.5}}  onPress={() => {setColor(item.light); onClose()}}></TouchableOpacity>
                            <TouchableOpacity style={{width: '33%', height: 50, borderRadius: 15, backgroundColor: item.color, marginLeft: 2.5, marginRight: 2.5}}  onPress={() => {setColor(item.color); onClose()}}></TouchableOpacity>
                            <TouchableOpacity style={{width: '33%', height: 50, borderRadius: 15, backgroundColor: item.dark, marginLeft: 2.5, marginRight: 2.5}} onPress={() => {setColor(item.dark); onClose()}}></TouchableOpacity>
                        </View>
