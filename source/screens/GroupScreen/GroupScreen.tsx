...
export const GroupScreen = ({navigation, route}: GroupScreenProps) => {
  ...
  const History = () => <HistoryScreen navigation={navigation as StackNavigationProp<RootStackParamList, 'History'>} route={{key: 'History', name: 'History', params: {group: group}}} />;

  const Leaderboard = () => <LeaderboardScreen group={group} />;

  const Events = () => <GroupEventsScreen navigation={navigation as StackNavigationProp<RootStackParamList, 'GroupEvents'>} route={{key: 'GroupEvents', name: 'GroupEvents', params: {group: group}}} />;
  ...
  const acceptUser = (userId: string) => {
    if (selectedGroup != null) {
      realm.write(() => {
        selectedGroup[0].members.push(userId), selectedGroup[0].memberRoles.push('player');
    ...
    <>
      ...
      <>
        <View style={styles.pageVisualiser}>
          <TouchableOpacity onPress={() => handlePageChange(0)} style={[styles.bar, selectedPage == 0 && {backgroundColor: colors.green}, selectedGroup[0].color != null && selectedPage == 0 && {backgroundColor: selectedGroup[0].color}]}>
            <Text style={selectedPage == 0 && {color: 'white', fontWeight: '800'}}>Events</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => handlePageChange(1)} style={[styles.bar, selectedPage == 1 && {backgroundColor: colors.green}, selectedGroup[0].color != null && selectedPage == 1 && {backgroundColor: selectedGroup[0].color}]}>
            <Text style={selectedPage == 1 && {color: 'white', fontWeight: '800'}}>History</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => handlePageChange(2)} style={[styles.bar, selectedPage == 2 && {backgroundColor: colors.green}, selectedGroup[0].color != null && selectedPage == 2 && {backgroundColor: selectedGroup[0].color}]}>
            <Text style={selectedPage == 2 && {color: 'white', fontWeight: '800'}}>Leaderboard</Text>
          </TouchableOpacity>
        </View>
        <PagerView
      ...
      <Modal isVisible={modalUsersVisible} swipeDirection={['down']} onSwipeComplete={closeUsersModal} onBackdropPress={closeUsersModal} style={styles.modalView}>
        <View style={styles.modalContent}>
                ...
                <View key={new BSON.ObjectID().toString()} style={styles.userInfoContainer}>
                  <View style={styles.userData}>
                    <View>
                      ...
                      {getUserStatus(item) == 'Injured' && <MaterialCommunityIcons name="circle" size={25} color={colors.red} style={{position: 'absolute', bottom: 0, right: 10}} />}
                      {getUserStatus(item) == 'Away' && <MaterialCommunityIcons name="circle" size={25} color={colors.orange} style={{position: 'absolute', bottom: 0, right: 10}} />}
                    </View>

                    <View>
                      <Text style={styles.username}>{getUserName(item)}</Text>
                      <Text style={styles.role}>{getUserRole(item)}</Text>
                      {isOwner(groupId, item) && <Text style={styles.role}>owner</Text>}
                    </View>
                  </View>
                  <View>
                    <TouchableOpacity
